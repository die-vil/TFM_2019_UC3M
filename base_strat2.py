...
Created on Wed Aug 21 09:11:09 2019
...
def estrategia_basica_2(prices, prices_train):
    ...
    for i in range(0, prices_train.shape[0]):
        ...
        #regr.fit(prices_train[etfs[1]].values.reshape(-1,1), prices_train[etfs[0]].values.reshape(-1,1))            
        ...
        #yhat = regr.predict(prices_train[etfs[1]].values[i].reshape(1,-1))
    ...
    for i in range(0, prices.shape[0]):
        ...
        #regr.fit(datos_entrenamiento[etfs[1]].values.reshape(-1,1), datos_entrenamiento[etfs[0]].values.reshape(-1,1))
        ...
        #yhat = regr.predict(prices[etfs[1]].values[i].reshape(1,-1))
